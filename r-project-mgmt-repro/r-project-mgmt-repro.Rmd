...
![](images/02-name-new-rmd.png){width="4.5in"}

Figure 12: Creating an RMD file in RStudio.

Give it a title: **Introduction to R Markdown**
...
Figure 13: Creating an RMD file in RStudio.
...
[Knitr](https://yihui.org/knitr/) is the engine in RStudio which creates the "dynamic" part of R Markdown reports. It's specifically a package that allows the integration of R code into the html, word, pdf, or LaTex document you have specified as your output for R Markdown. There are two main ways to process code with Knitr in R Markdown documents:
...
There's a button you can use in the RStudio menu to generate the code chunks automatically. Automatic code chunk generation is available for several other languages as well. Also, you can use the keyboard shortcut `ctrl`+`alt`+`i` for Windows and `command`+`option`+`i` for Mac.

![](images/code-chunk-script.png){width="6.5in"}

As you can see in Figure xx, the console is blank because we have not executed/run the code chunk.

## **Run the code in a code chunk**

We have options for running and debugging code that don't require us to wait for the file to render. **START HERE IN THE AM**
