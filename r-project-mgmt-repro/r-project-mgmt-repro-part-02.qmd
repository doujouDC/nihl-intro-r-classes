...
There are two ways to use Quarto. The first method involved installing the Quarto application. Quarto is available in a number of different operating systems \[Figure 1\].

![](images/quarto-download.png){width="640"}

Figure 1: Quarto download options.

The next step is choosing your authoring tool \[Figure 2\].

![](images/quarto-supported-tools.png){width="640"}

Figure 2: Authoring tools supporting Quarto.

The second option is using the Quarto by downloading the `quarto` package. The [latest release](https://rstudio.com/products/rstudio/download/) of RStudio (v2022.07) includes support for editing and preview of Quarto documents.
...
If you have already installed the Quarto CLI then RStudio will detect this and enable Quarto features automatically. Alternatively, there is a version of Quarto built-in to RStudio that you can activate from R Markdown Preferences \[Figure 3\]. You will be prompted to restart R after you select **Reply**. So, save your work.

![](images/quarto-settings-rstudio.png){width="640"}

Figure 3: Activating Quarto within RStudio.

# Packages

Like the rest of R, R Markdown, Knitr, and Quarto are free and open source. You can install the R Markdown package from CRAN with:
