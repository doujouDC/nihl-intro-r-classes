...
**\[with this content, I need to decide if I am going to stick the content below. Or, implement #\| to support multiple languages\]**
...
Let us revisit the Quarto workflow that we saw in Figure 2, focusing on the code block \[Figure 14\].
...
R markdown adopts Pandoc\'s Fenced Code Block, which starts and stops the code chunk with three back-ticks.

| \`\`\`{r}
| rnorm(3)
| \`\`\`

So, the code code chunk below implements [rnorm](https://www.rdocumentation.org/packages/compositions/versions/2.0-4/topics/rnorm), which generates a vector of normally distributed random numbers.
