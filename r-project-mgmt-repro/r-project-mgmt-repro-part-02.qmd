...
Although I am demonstrating using R, it's possible to use other programming or markup languages. For example, we have seen that we can use LaTeX code for equations. You can also use python and a handful of other languages \[Figure 15\].

![](images/code-chunk-other.png){width="6.5in"}

Figure 15: Code options available in R-markdown.
...
You might have noticed that there are some special comments at the top of the code block. These comments use `#| (the hash pipe!)`. `#|` provides the cell options. When the document is rendered, this code chunk will look like this [Figure 16]:

![](images/quarto-code-chunk-rendered.png){width="640"}

Figure 16: Rendered version of a code-chunk.
...
1)  Run from code chunk (green play button on the right top corner). This allows us to run one specific code chunk [Figure 17].

![](images/code-chunk-run-01.png){width="6.5in"}

Figure 17: Running a code chunk from the code-box.

2.  Run menu, which gives more options for running code chunks including the current one, the next one, all chunks, etc [Figure 18].

![](images/code-run-chunk-02.png){width="6.5in"}

Figure 18: Running a code chunk from the Code-Chunk menu.
...
Figure 18 is displaying the output of the code-chunk. Can you explain what is happening in the console?

![](images/knit-02.png){width="6.5in"}

Figure 18: Output from our first code chunk.
...
While it is possible to use R chunks options, hash pipe options are more portable across computational engines. Also, don't forget the "tab-complete" option that is built into RStudio [Figure 19].

![](images/code-yaml-auto-complete.png){width="640"}

Figure 19: Example of tab-complete feature in RStudio.
...
The code chunk below is using R markdown.
