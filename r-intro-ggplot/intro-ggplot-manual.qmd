...
You begin every plot by telling the `ggplot()` function what your data is. We are going to import data on New York City restaurant inspection [data](https://data.cityofnewyork.us/Health/DOHMH-New-York-City-Restaurant-Inspection-Results/43nn-pn8j). This dataset is huge, so I have filtered by: Year \[2022\] and Burrough \[Manhattan\]

```{r}
nyc_rest_inspection_manhattan <- read_csv("../raw-data/nyc-rest-inspection-manhattan.csv", col_types = cols(ZIPCODE = col_character(), PHONE = col_character(), `INSPECTION_DATE` = col_date(format = "%D"), 
`RECORD_DATE` = col_date(format = "%D")))
```

`ggplot(data = nyc_rest_inspection_manhattan)`
...
```{r}
#| output: false
glimpse(nyc_rest_inspection_manhattan)
```

We are going to plot the following variables from the nyc data, which we will place inside the mapping function. Let us plot `grade` against `score`.
...
```{r}
nyc_rest_inspection_manhattan %>% 
  ggplot(mapping = aes(x = GRADE, y = SCORE))
```

As we can see in the output above, `aes()` allows us to select the variables to be plotted and specify how to present them in the graph. 

STOPPED, PICK UP AFTER LUNCH
