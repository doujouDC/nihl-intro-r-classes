...
- [`geom_raster()`](https://ggplot2.tidyverse.org/reference/geom_tile.html) draw rectangles
- [`geom_rect()`](https://ggplot2.tidyverse.org/reference/geom_tile.html) is parameterised by the four corners of the rectangle, `xmin`, `ymin`, `xmax` and `ymax`
-  [`geom_tile()`](https://ggplot2.tidyverse.org/reference/geom_tile.html) is exactly the same, but parameterised by the center of the rect and its size, `x`, `y`, `width` and `height`
-   [`geom_raster()`](https://ggplot2.tidyverse.org/reference/geom_tile.html) is a special case of [`geom_tile()`](https://ggplot2.tidyverse.org/reference/geom_tile.html) used when all the tiles are the same size.
...
As previously noted, the general structure of the code that is used to plot figures is as follows:
...
4.  Using + layer on geoms as needed, by adding them to the object one at a time.

5.  Use some additional functions to adjust the scales, labels, tick marks, and titles.

STOPPED ON OCT 9

Let us start by returning to the gapminder data, and saving our ggplot statement to an object named p1
