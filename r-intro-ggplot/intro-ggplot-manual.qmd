...
You begin every plot by telling the `ggplot()` function what your data is. For example:
...
We are going to plot the following variables from the gapminder data, which we will place inside the mapping function. Using the Gap Minder data, let us plot life expectancy against per capita GDP for all country-years in the data.
...
As we can see in the output above, `aes()` allows us to select the variables to be plotted and specify how to present them in the graph. What does this look like in ggplot? These are some possible options:
