...
According to the [U.S. National Science Foundation (NSF)](https://www.nsf.gov/sbe/AC_Materials/SBE_Robust_and_Reliable_Research_Report.pdf) subcommittee on reproducibility in science [@bollenSocialBehavioralEconomic2015]:

Science should routinely evaluate the reproducibility of findings that enjoy a prominent role in the published literature. To make reproduction possible, efficient, and informative, researchers should sufficiently document the details of the procedures used to collect data, to convert observations into analyzable data, and to perform data analysis [@wilson2016].
...
See if you can find a Wilson article in the bib from 2017

Adapted from [@wilsonSoftwareCarpentryLessons2014; @millsArchivingPrimaryData2015; @tealDataCarpentryWorkshops2015; @wickhamWelcomeTidyverse2019]:

### Don't Touch the Raw Data

Don't ever do analysis on the original data, or, if you do, then explain what you did to the data [@federerDataLiteracyTraining2016].
...
You cannot find your files on your computer (or your cloud storage) Multiple versions of files with names such as "finaldraft_4.txt" Path issues when trying to run code Reviewers or colleagues cannot re-run your code/analyses
...
6.  Programs brought in from elsewhere or compiled locally in the bin directory Name all files to reflect their content or function
